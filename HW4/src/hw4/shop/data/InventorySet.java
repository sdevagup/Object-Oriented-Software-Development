...
final class InventorySet implements Inventory {
  ...
  public Record get(Video v) {
    // TODO
	  if (_data.containsKey(v)) { 
		  RecordObj rec = (RecordObj) _data.get(v); 
		  return rec; 
      }
    return null;
  }
  ...
  public Iterator<Record> iterator(Comparator<Record> comparator) {
    // TODO
    //return null;
	  Collection<Record> values = _data.values();
	  ArrayList<Record> listOfValues = new ArrayList<Record>(values);
	  Collections.sort(listOfValues, comparator);
	  
    return Collections.unmodifiableCollection(listOfValues).iterator();
  }
    ...
    // TODO
	  ...
	  if(video != null && change !=0) {
		  if (_data.containsKey(video)) { 
			  ...
			  RecordObj val = new RecordObj(video,change,0,0);
			  _data.put(video, val);  }
	  }
...
RecordObj record = (RecordObj) _data.get(video);
		  int val = record.numOut() + 1;
		  
		  if(val <=  record.numOwned()) { 
			  record.numOut += 1;
			  record.numRentals += 1;
		  }
		  else if (val >  record.numOwned()) 
