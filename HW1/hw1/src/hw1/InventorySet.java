    ...
    // TODO  
	  if(video != null && change !=0) {
		  if (_data.containsKey(video)) { 
			  ...
			  Record a = new Record(video,change,0,0);
			  _data.put(video, a); 
		  }
	  }
	  else {
  ...
  public void checkOut(VideoObj video) {
    // TODO  
	  if (_data.containsKey(video)) { 
		  ...
		  if(a <  rec.numOwned) 	//numOut < numOwned
			  rec.numOut += 1;
		  
		  else if (a ==  rec.numOwned) 	//numOut = numOwned
  ...
  public void checkIn(VideoObj video) {
    // TODO  
	  if (_data.containsKey(video)) { 
		  ...
		  if(a >= 0) 	//numOut not negative
			  rec.numOut += -1;
		  
		  else //numOut is negative
			  throw new IllegalArgumentException("numOut is negative");
